...
begin
  // Define the datetime format for the filename
  DateTimeFormat := 'yyyymmdd_hhnn';
  // Validate Parameter
  ...
  begin
    ...
    // Prepare CSV file
    CSVFileName := EditOutputFolder.Text + '\DS10EAC100_Plan_' + FormatDateTime(DateTimeFormat, Now)  +'.csv';
  end
  else
  begin
    // SQL Text Generate
    ...
    // Prepare CSV file
    CSVFileName := EditOutputFolder.Text + '\DS10EAC100_Act_' + FormatDateTime(DateTimeFormat, Now) + '.csv';
  end;
  // Execute SQL
  UniQuery.Open;
